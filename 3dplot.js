function render3d() {
    ...
    var svg    = d3.select('#errorSurface').call(d3.drag().on('drag', dragged).on('start', dragStart).on('end', dragEnd)).append('g');
    ...
    function processData(data, tt){
        ...
        points
            ...
            .attr('r', 3)
            .attr('stroke', function(d){ return d3.color(color(d.error)).darker(3); })
            .attr('fill', 'green')
            .attr('opacity', 1)
            .attr('cx', posPointX)
            .attr('cy', posPointY);
	...
	function init(){
        ...
        for(var z = -j; z < j; z++){
            for(var x = -j; x < j; x++){
                ...
                y = -0.1 * perceptron.computeTotalErrorAt(points, x/100.0, z/100.0);
                // y = d3.randomUniform(0, 10)();
                scatter.push({x: x, y: y, z: z, id: 'point_' + cnt++});
            }
        }
